...
import {
  chatSessionApp,
  historyPage,
  selectedElementFixture,
} from "./chat-session-test-fixtures";
...
describe("useChatSession", () => {
  ...
  it("keeps current generation messages when older history is appended", async () => {
    const { user, rerender } = renderWithProviders(
      <Harness historyRecords={historyPage(1)} />,
    );
    ...
    rerender(
      <Harness historyRecords={[...historyPage(1), ...historyPage(3)]} />,
    );
  ...
  it("blocks generation when the current user cannot manage the app", async () => {
    const { user } = renderWithProviders(
      <Harness historyRecords={[]} canGenerate={false} />,
    );
  ...
  it("does not auto-send the initial prompt for admin viewers", () => {
    renderWithProviders(
      <Harness historyRecords={[]} autoSendInitialPrompt={false} />,
    );
