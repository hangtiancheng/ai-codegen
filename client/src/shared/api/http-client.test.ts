...
describe("createHttpClient", () => {
  ...
  it("builds URL with query params", async () => {
    ...
    await client.request(
      { method: "GET", url: "thing", query: { id: 1 } },
      dataSchema,
    );
  ...
  it("throws business error on non-zero code", async () => {
    fetchSpy.mockResolvedValueOnce(
      envelope({ code: 40000, message: "bad request" }),
    );
    const client = createHttpClient(() => baseUrl);
    await expect(
  ...
  it("classifies 5xx as http error", async () => {
    ...
    await expect(
      client.request({ method: "GET", url: "thing" }, dataSchema),
    ).rejects.toMatchObject({ error: { kind: "http", status: 500 } });
  });
  ...
  it("classifies invalid JSON as parse error", async () => {
    ...
    await expect(
      client.request({ method: "GET", url: "thing" }, dataSchema),
    ).rejects.toMatchObject({ error: { kind: "parse" } });
  ...
  it("classifies AbortError as aborted", async () => {
    ...
    await expect(
      client.request(
        { method: "GET", url: "thing", signal: controller.signal },
        dataSchema,
      ),
