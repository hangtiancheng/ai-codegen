...
const baseEnvSchema = z.object({
  ...
  RATE_LIMIT_AI_GENERATION_WINDOW_SECONDS: z.coerce
    .number()
    .int()
    .min(1)
    .default(60),
  ...
  SESSION_SECRET: z
    .string()
    .min(16)
    .default("server-v2-default-secret-change-me"),
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.SCREENSHOT_BROWSER_EXECUTABLE_PATH === undefined) {
      ctx.addIssue({
        code: "custom",
        message:
          "SCREENSHOT_BROWSER_EXECUTABLE_PATH must be configured in production",
