...
import { CODE_QUALITY_CHECK_SYSTEM_PROMPT, getSystemPrompt } from "../ai/index";
import type { AiModelRegistry } from "../ai/models/index";
import { describeViteOutputCompletenessIssue } from "./vite-output-completeness";
import type { CodeGenerator, CodegenStreamMetadata, QualityChecker } from "./workflow-service";
...
export const createLangChainCodeGenerator = (registry: AiModelRegistry): CodeGenerator => ({
  streamCode: async function* ({ codegenType, prompt }) {
    const model = registry.createModel("streaming");
    const stream = await model.stream([
      new SystemMessage(getSystemPrompt(codegenType)),
