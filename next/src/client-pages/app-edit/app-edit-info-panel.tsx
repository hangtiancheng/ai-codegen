  ...
  return (
    <section className="border-border bg-card grid gap-4 rounded-2xl border p-5 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <InfoTile label="Deployed" value={app.deployTime ? formatDateTime(app.deployTime) : "No"} />
...
function InfoTile({
  ...
  return (
    <div className="border-border bg-background rounded-xl border p-3">
      ...
      <div className="text-foreground mt-1 min-h-6 text-sm font-medium">{children ?? value}</div>
    </div>
  );
}
