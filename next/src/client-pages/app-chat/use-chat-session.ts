...
import type { AppVo, ChatHistory } from "@/shared/schemas";
import {
  type ChatMessage,
...
export function useChatSession(
  ...
  const [sessionMessages, setSessionMessages] = useState<ReadonlyArray<ChatMessage>>([]);
  ...
  const runPrompt = useCallback(
    (message: string): void => {
      ...
      const prompt = appendSelectedElementContext(message, visualEditor.selectedElement);
      ...
      void runChatStream(
        { url: buildChatStreamUrl(app.id, prompt), signal: controller.signal },
        {
          onChunk: (chunk) => {
            content += chunk;
            setSessionMessages((current) => replaceMessage(current, aiMessage.id, content, false));
          },
          onDone: () => {
            setGenerating(false);
            onPreviewReady();
  ...
  useEffect(() => {
    if (!app || !autoSendInitialPrompt || !historyLoaded || autoSentRef.current) return;
