...
import { type AppVo, appIdSchema } from "@/shared/schemas";
...
vi.mock("./app-chat-workspace", async () => {
  ...
  return {
    AppChatWorkspace: ({ app }: { readonly app: AppVo }) => {
      ...
      return React.createElement("p", {}, `initial ${initialId} current ${app.id}`);
    },
  };
});
...
describe("AppChatContent", () => {
  ...
  it("remounts the chat workspace when the app id changes", async () => {
    ...
    const { rerender } = renderWithProviders(<AppChatContent appId={appIdSchema.parse(1)} />);
