...
import { useDeleteApp, useDeleteAppByAdmin, useDeployApp } from "@/shared/query";
...
export function AppChatWorkspace({ app }: { readonly app: AppVo }): ReactNode {
  ...
  const activeDeployKey = app.deployTime ? (app.deployKey ?? undefined) : undefined;
  const shouldAutoSendInitialPrompt = isOwner && activeDeployKey === undefined;
  const previewReady =
    activeDeployKey !== undefined || history.records.length >= 2 || generatedPreviewReady;
