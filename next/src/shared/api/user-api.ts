...
export async function logout(): Promise<boolean> {
  return httpClient.request({ method: "POST", url: "user/logout" }, z.boolean());
}

export async function getCurrentUser(): Promise<LoginUserVo> {
  return httpClient.request({ method: "GET", url: "user/get/login" }, loginUserVoSchema);
}

export async function updateUserProfile(body: UserUpdateRequest): Promise<boolean> {
  return httpClient.request(
    {
      method: "POST",
      url: "user/update",
