...
export function parseStreamEvent(raw: RawStreamEvent): ParsedStreamEvent {
  ...
  if (eventName === chatStreamEventNames.businessError) {
    const parsed = chatStreamBusinessErrorPayloadSchema.safeParse(parseJsonSafely(raw.data));
    if (parsed.success) {
  ...
  if (eventName === chatStreamEventNames.message) {
    const parsed = chatStreamMessagePayloadSchema.safeParse(parseJsonSafely(raw.data));
    if (parsed.success) {
