...
import { envelope, requestBodyContains, requestedUrlContains } from "@/test/http-test-helpers";
import {
  addApp,
  deleteAppByAdmin,
  deployApp,
...
describe("app-api", () => {
  ...
  it("loads app details with a query string", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "app/get/vo?id=10")).toBe(true);
  });
  ...
  it("parses app ids returned as strings after add", async () => {
    ...
    await expect(addApp({ initPrompt: "Build a dashboard" })).resolves.toBe("100");
  ...
  it("uses admin list and update endpoints", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "app/admin/update")).toBe(true);
    expect(requestBodyContains(fetchSpy.mock.calls, '"priority":99')).toBe(true);
  });
  ...
  it("uses the configured awesome app endpoint", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "app/awesome/list/page/vo")).toBe(true);
  });
  ...
  it("uses my app endpoint with unauthorized redirect suppressed", async () => {
    fetchSpy.mockResolvedValueOnce(envelope({ code: 40100, message: "User not logged in" }, 401));

    await expect(listMyAppPage({ pageNum: 1, pageSize: 10 })).rejects.toThrow();
    expect(requestedUrlContains(fetchSpy.mock.calls, "app/my/list/page/vo")).toBe(true);
  });

  it("normalizes legacy app list payload fields", async () => {
    fetchSpy.mockResolvedValueOnce(envelope({ code: 0, data: legacyAppPage() }));
  ...
  it("deletes and deploys apps through validated endpoints", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "app/admin/delete")).toBe(true);
