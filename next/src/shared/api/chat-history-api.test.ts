...
import { listAdminChatHistoryPage, listAppChatHistory } from "./chat-history-api";
...
describe("chat-history-api", () => {
  ...
  it("loads app chat history through cursor query params", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "chat-history/app/10")).toBe(true);
    expect(requestedUrlContains(fetchSpy.mock.calls, "pageSize=20")).toBe(true);
    expect(requestedUrlContains(fetchSpy.mock.calls, "cursor=2026-05-18T10")).toBe(true);
  });
  ...
  it("validates admin chat filters and parses page responses", async () => {
    ...
    expect(requestBodyContains(fetchSpy.mock.calls, '"messageType":"user"')).toBe(true);
