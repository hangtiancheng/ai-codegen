...
import { deleteUser, getCurrentUser, listUserPage, login, register } from "./user-api";
...
describe("user-api", () => {
  ...
  it("parses the register id returned as a string", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "user/register")).toBe(true);
  });
  ...
  it("sends typed admin list and delete requests", async () => {
    ...
    expect(requestedUrlContains(fetchSpy.mock.calls, "user/list/page/vo")).toBe(true);
