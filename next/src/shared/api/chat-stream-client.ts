...
import { type ParsedStreamEvent, parseStreamEvent } from "./chat-stream-parser";
...
const defaultEngine: StreamEngine = async (url, init) => {
  await fetchEventSource(url, {
    ...
    onmessage: (event) => init.onmessage({ event: event.event, data: event.data }),
    ...
    await engine(request.url, {
      ...
      onerror: (cause) => {
        ...
        if (cause instanceof DOMException && cause.name === "AbortError") {
          handlers.onError(new ApiException({ kind: "aborted", message: "stream aborted" }));
          return;
        }
        handlers.onError(
          new ApiException({
